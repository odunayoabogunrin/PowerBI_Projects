# Recruitment Analytics Dashboard

## Overview

This project showcases an interactive Recruitment Analysis Dashboard built in Microsoft Power BI to monitor and optimize hiring performance across business units, recruiters, departments, and recruitment channels.

The dashboard provides HR teams and leadership with actionable insights into:

- Hiring progress

- Recruitment pipeline efficiency

- Source effectiveness

- Position fulfillment timelines

- Candidate tracking

- Hiring KPIs

The solution is designed to support data-driven hiring decisions and improve recruitment operations.


## Dashboard Preview


### Positions Overview Dashboard


### Interviews & Candidate Tracking Dashboard


## Business Problem

Recruitment teams often struggle with:

- Tracking hiring progress across multiple departments

- Measuring recruiter performance

- Identifying bottlenecks in hiring timelines

- Monitoring candidate pipeline stages

- Understanding which recruitment sources generate quality hires


This dashboard addresses these challenges by centralizing recruitment data into a single interactive reporting solution.

### Objectives

The main objectives of this project are to:

- Monitor recruitment KPIs in real time

- Analyze hiring trends and timelines

- Track open vs completed positions

- Evaluate recruiter and department performance

- Identify effective candidate sourcing channels

- Improve hiring completion rates

- Enable faster HR decision-making


## Key Features

**1. Executive KPI Cards**

The dashboard includes high-level recruitment metrics such as:

- Total Positions

- Total Applications

- Total Hires

- Hiring Completion Percentage

**2. Hiring Status Analysis**

Visualizes:

- Hires completed by business unit

- Position fulfillment status

- Over-hired vs under-hired departments

- Remaining hiring balance

**3. Recruitment Timeline Analysis**

Tracks:

- Applications over time

- Hiring trends across weeks

- Recruitment pipeline progression

-Time-to-hire insights

**4. Candidate Source Analysis**


Analyzes candidate inflow from:

- LinkedIn

- Naukri

- Monster

- Referral platforms

- Other sourcing channels


This helps HR identify the most effective recruitment platforms.


**5. Recruiter Performance Monitoring**

Enables comparison of:

- Recruiter workload

- Hiring efficiency

- Position closure rates

- Recruitment productivity


**6. Interactive Filtering**

Users can dynamically filter data by:

- Level/Band

- Recruiter

- Department

- Location

- Status

- Hiring Type

- Time Period

### Tools & Technologies
**Tool and Purpose**

Power BI   - Data visualization & dashboard creation

Power Query - Data transformation & cleaning

DAX  - KPI calculations & measures

Excel / CSV - Data source

HR Analytics - Business domain


## Dashboard Pages

### 1. Positions Dashboard









Focuses on:

- Open positions

- Hiring progress

- Department-wise hiring

- Position timelines

**Insights Included**

- Hiring completion rate

- Positions vs hires

- Hiring backlog

- Business-wise recruitment distribution

### 2. Interviews Dashboard


Focuses on:

- Candidate pipeline tracking

- Interview stages

- Source effectiveness

- Candidate status monitoring

**Insights Included**

- Candidate sources

- Interview progress

- Joining status

- Candidate qualification tracking


## Data Model

The dashboard uses a relational HR recruitment dataset containing:

- Candidate information

- Job positions

- Recruiter data

- Hiring status

- Department records

- Source channels

- Timeline metrics

**Example Fields**

- Req ID

- Position Title

- Department

- Recruiter

- Candidate Name

- Status

- Applications

- Hires

- Open Days

- Joining Date

### KPIs Implemented
|KPI                        |     Description     |
---------------------------------------------------------------



Hiring Completion %

Measures completed hires against target

Total Applications

Total candidate applications received

Total Hires

Successful hires completed

Open Positions

Remaining positions to fill

Balance Hires

Difference between target and actual hires

Source Effectiveness

Performance of recruitment platforms

Skills Demonstrated


This project demonstrates expertise in:

HR Analytics

Dashboard Design

Data Storytelling

KPI Development

DAX Calculations

Data Cleaning

Business Intelligence

Interactive Reporting

Recruitment Analytics

Business Impact


This dashboard can help organizations:

Reduce hiring delays

Improve recruiter accountability

Optimize recruitment channels

Monitor hiring performance centrally

Increase recruitment efficiency

Support workforce planning decisions

Future Improvements


Potential enhancements include:

Predictive hiring analytics

AI-powered recruitment forecasting

Attrition integration

Diversity hiring metrics

Automated alerts

Real-time HR database integration

How to Use
Clone the repository

Open the .pbix file in Power BI Desktop

Refresh the dataset if required

Interact with filters and visuals

Use Cases


This dashboard is useful for:

HR Teams

Talent Acquisition Teams

Recruitment Agencies

Workforce Planning

HR Business Partners

Executive Leadership

Project Highlights for Portfolio
Designed a multi-page interactive HR analytics dashboard in Power BI

Built KPI-driven recruitment monitoring solution

Implemented DAX measures for hiring analytics

Developed interactive drill-down reporting experience

Analyzed recruitment funnel and sourcing effectiveness

Enabled business users to monitor hiring performance in real time

Author


Odunayo Abogunrin

Data Analyst | Business Intelligence Analyst | HR Analytics 



Connect With Me
LinkedIn: Add your LinkedIn profile

Portfolio: Add your portfolio link

Email: Add your professional email

